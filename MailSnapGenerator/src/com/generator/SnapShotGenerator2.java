...
public class SnapShotGenerator2 {

	public static void main(String[] args) throws IOException, InterruptedException {
		SnapShotGenerator2 snapShotGenerator = new SnapShotGenerator2();
		if (args.length > 1)
	...
	private void takeSnapshot(String... ersReportparam) {
		...
		WebDriver driver = WebDriverUtil.createWebDriverInstance("CHROME");
		driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
		driver.manage().window().maximize();
		driver.get(path + ersReportparam[1]);
		driver.findElement(By
				.xpath("//a[contains(@view,'dashboard-view')]/i[contains(@class,'material-icons') and contains(.,'track_changes')]"))
				.click();
		try {
			Thread.sleep(1000);
		...
		driver.manage().window()
				.setSize(new Dimension(Integer.parseInt(ersReportparam[2]), Integer.parseInt(ersReportparam[3])));
		try {
			Thread.sleep(2000);
		} catch (InterruptedException e1) {
			e1.printStackTrace();
		...
		driver.close();
		String htmlContent = "<html><body><img width='" + ersReportparam[5] + "' height='" + ersReportparam[6]
				+ "' src='data:image/png;base64,"
						.concat(EncryptionUtility
								.encodeFileToBase64Binary(new File(ersReportparam[4].concat("/mailSnap.png"))))
						.concat("'/></body></html>");
	...
	private void takeSnapshot(String mailPath) throws IOException, InterruptedException {
		...
		try {
			...
			BufferedImage eleScreenshot = fullImg.getSubimage(webElement.getLocation().getX(), 0,
					webElement.getSize().getWidth(), webElement.getSize().getHeight() + 50);
			ImageIO.write(eleScreenshot, "png", screenshot);
			File screenshotLocation = new File(mailPath + "\\screenshot.png");
			FileUtils.copyFile(screenshot, screenshotLocation);
			String htmlContent = "<html><body><img width='" + "100%" + "' height='" + "100%"
					+ "' src='data:image/png;base64,"
							.concat(EncryptionUtility
									.encodeFileToBase64Binary(new File(mailPath.concat("/screenshot.png"))))
							.concat("'/></body></html>");
